...
- ``*args`` and ``**kwargs`` in decorated function signatures are ignored. Configs are passed as positional arguments to regular parameters only. Functions with ``*args`` or ``**kwargs`` will not crash, but these parameters will always be empty and will not appear in the profiling output.
