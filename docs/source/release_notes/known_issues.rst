...
- Only regular positional parameters are supported in decorated function signatures. ``*args`` and ``**kwargs`` at the end of the signature are tolerated but ignored — they will always be empty and will not appear in the profiling output. Keyword-only parameters (those after ``*`` or ``*args``) are not supported and will cause a runtime error.
